...
export default function FinancePanel() {
  ...
  const exportFile = async (format: "csv" | "pdf") => {
    if (!data) return;
    try {
      const res = await fetch(`/api/membership/analytics/export.${format}?year=${data.year}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      ...
      a.href = url;
      a.download = `doapp-${format === "csv" ? "facturacion" : "resumen"}-${data.year}.${format}`;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
          <div className="flex items-center justify-between gap-2 mb-3">
            ...
            <div className="flex items-center gap-2" title={t("finance.exportHelp", "Descargá tus facturas del año para tu contador.")}>
              <span className="text-xs text-slate-400 hidden sm:inline">{t("finance.exportFor", "Para el contador:")}</span>
              <button
