...
export const termsEn: Record<string, string> = {
  ...
  "s6p7": `6.7. <b>Reviews and rating.</b> When a contract ends, each party rates the other and writes a public review, which is shown on the rated person's profile and is required to keep using the Platform. Optionally, they may also leave a private note, which only the rated person and the Platform's administration can read; it is not published and does not change the rating. The rating shown on a profile is computed from reviews in the last ${P.RATING_VENTANA_DIAS} days when that period holds at least ${P.RATING_CONTRATOS_MINIMOS} reviews; otherwise from the last year, and failing that from the whole history. The period used is stated next to the rating. Reviews must refer to the experience in the contract; the Platform may hide those containing personal data, insults or content unrelated to the work.`,
  ...
  "s7p3": `7.3. DOAPP charges a ${COMISION}% commission on the job price, paid by the Client and shown before every payment. The commission is the same with or without a membership: the membership buys visibility (section 8), not discounts. During the launch period (beta), whose end date is shown on the Platform, the commission is 0%; afterwards the stated rate applies. The payment processing cost (section 7.10) is independent of the commission and is charged during the beta as well. The following table summarises the commission per plan:`,
  "thPlan": "Plan",
  "thCommission": "Commission",
  "planProMonth": `PRO (€${PRO_EUR}/month, at the day's exchange rate)`,
  "s7p4": `7.4. <b>Minimum commission.</b> The commission has a floor equivalent to EUR ${MINIMUM_COMMISSION_EUR}, converted to pesos at the day's exchange rate and shown before paying. The floor covers the fixed cost each contract has for the Platform. There is no minimum job amount: the Client may publish any amount, and on small jobs the minimum commission may represent a larger share of the price, which is disclosed before paying.`,
  "s7p5": `7.5. DOAPP's commission is non-refundable once a Worker has been selected for the publication, even in cases of cancellation or dispute. If the publication is cancelled without a Worker ever having been selected — before or after its approval — the commission is returned as Platform credit under section 9.1, and ${REVISION_PARTE_PCT}% of the commission is retained when that credit is transferred out of the Platform.`,
  "s7p6": `7.6. <b>Automatic release due to Client absence:</b> If a job ends (due date reached) and the Client does not confirm receipt of the service within the following ${P.AUTO_CONFIRMACION_HORAS} hours, the payment held in custody will be automatically released to the assigned Workers. The Platform commission corresponding to the Client's plan is retained in all cases. The Client will receive notifications before and during this process. <b>The actual transfer to the Worker takes place once the claim period of section 10.7 has expired</b>: releasing the payment means the Client no longer controls it, not that the money has left the Platform. This clause does not apply if there is an active dispute over the contract.`,
  ...
  "s7p8": "7.8. <b>Minimum extension amount.</b> The Platform sets a minimum amount for contract extensions, derived from the gateway processing cost and the fixed cost each operation carries for the Platform. The amount in force is disclosed before confirming the extension.",
  "s7p9": "7.9. <b>Credit balance, refunds and transfers.</b> When amounts are to be returned to the Client — from a quote below the published price, a cancellation or a rejected publication — <b>the Client chooses between two routes, disclosed before confirming</b>: (a) <b>Platform credit</b>, usable at no cost on any subsequent publication or hiring, whose portion paid with credit does not go through the gateway and carries no processing cost; or (b) a <b>refund to the payment method used</b>, whose crediting time depends on the issuer. Platform credit may also be transferred later to a bank account in the Client's name. Both the refund to the payment method and the bank transfer mean the money leaves the Platform, and in both cases the share of the commission indicated in section 9.1 is deducted where applicable; apart from that deduction, <b>neither carries any charge</b>. If the Client does not accept that deduction, the balance remains available on the Platform indefinitely.",
  "s7p10": "7.10. <b>Payment processing cost.</b> Every payment entering the Platform through the gateway carries a processing cost, <b>borne by the Client</b>, calculated with a single rate on the total paid, the same for every payment method, and disclosed together with its VAT before the payment is confirmed. The current rate is published on the Platform. <b>The Worker receives the full job price</b>: neither the commission nor the processing cost is deducted from it. <b>The processing cost is non-refundable</b> in every case — cancellation, rejected publication, dispute or refund — because the gateway charges it when the payment comes in and does not return it. The time it takes the gateway to make funds available may vary with the payment method chosen; the estimated transfer date to the Worker is shown in each contract.",
  ...
  "s9p1": `9.1. <b>Cancellations with no Worker selected</b> (before the publication is approved, or after approval while no Worker has been selected): the full amount paid, commission included and except for the payment processing cost (section 7.10), returns to the Client by the route they choose under section 7.9. If they choose Platform credit, it is credited in full and usable at no cost on new publications. If they choose a refund to the payment method, or later transfer that credit to a bank account, ${REVISION_PARTE_PCT}% of the commission with its VAT is deducted for the review already performed (with the current commission this equals ${REVISION_COMISION_PCT}% of the price, with a minimum of EUR ${REVISION_PISO_EUR}). A publication rejected by an Administrator is settled the same way, and the Administrator must state the reason.`,
  "s9p2": `9.2. <b>Client cancellations with a Worker selected and at least ${P.CANCELACION_CLIENTE_HORAS_ANTES} hours before the start</b>: the job price is credited as Platform credit; neither the publication commission nor the processing cost (section 7.10) is refunded. Transferring that credit to a bank account is free of charge (section 7.9).`,
  "s9p3": `9.3. <b>Late Client cancellations with a Worker selected</b> (less than ${P.CANCELACION_CLIENTE_HORAS_ANTES} hours before the start, or during execution): ${PARTE_TRABAJADOR_PCT}% of the price is paid to the Worker, in full, for the time reserved; the rest is credited to the Client as Platform credit. The publication commission and the processing cost are retained. If the Worker is the one who cancels — through the function provided in the Platform, before the start or during execution — the Worker receives nothing and is subject to section 9.4, and the Client chooses: keep the publication open with the funds already paid to select another Worker, republish it at a lower price (the difference is credited as Platform credit), or have the full job price credited as Platform credit. In all three cases the publication commission and the processing cost are retained, and transferring the credit is free of charge (section 7.9).`,
  ...
  "s10p10": `10.10. <b>Failure to respond in a dispute.</b> Each party has ${P.DISPUTA_DIAS_PARA_RESPONDER} calendar days to respond to the other party's last message within a dispute. Opening the dispute counts as the first message. If a party does not respond within that period, the dispute is passed to an Administrator for review with the recommendation to resolve it in favour of the party who responded last: if the non-responding party is the Worker, to return the held funds to the Client; if it is the Client, to release the payment to the Worker. <b>No movement of money is executed without an Administrator's express decision.</b> The Platform sends a notice to the silent party ${DISPUTA_AVISO_DIAS_ANTES} days before the deadline. Messages from the Platform's administration do not interrupt this period. The Platform commission is not refunded in any case, per section 7.5.`,
  "s10p11": `10.11. <b>Direct claim prior to Administrator intervention.</b> When a dispute is opened, a direct-claim period of ${P.RECLAMO_DIRECTO_HORAS} hours begins during which the parties may resolve the issue between themselves within the Platform: the party who opened the claim may withdraw it, and either party may propose returning all or part of the price to the Client, or that the Worker redo or complete the work. A proposal accepted by the other party closes the negotiation and is recorded as an agreement; <b>if it involves a refund, an Administrator executes it</b>, checking that it matches what was agreed before moving any money. Agreed refunds follow the same rules as sections 7.5 and 10.4. If the period expires without agreement, or if one party requests it once the other has already responded, the claim becomes a dispute and an Administrator intervenes under this section. Funds are frozen from the moment the claim is opened (section 10.2). The period in section 10.10 runs from the opening of the claim. The Platform may intervene before the period expires when it deems it necessary to protect Users or the Platform.`,
  ...
  "s11p3": "11.3. <b>Safety tools.</b> The Platform offers, free of charge, assistance tools such as the emergency button, the pairing code and the option to share a contract's details with a trusted contact. They are assistance tools that make it easier to reach emergency services or third parties chosen by the User; <b>they do not constitute a security, surveillance or response service</b>, and DOAPP does not guarantee the availability, response or outcome of any of them. The User is responsible for assessing the situation and contacting emergency services directly when appropriate.",
