...
export const termsEn: Record<string, string> = {
  ...
  "s7p5": `7.5. DOAPP's commission is non-refundable once the publication has been approved, even in cases of cancellation or dispute. The only exception is cancellation prior to approval (section 9.1), where ${100 - REVISION_PARTE_PCT}% of the commission is refunded.`,
  ...
  "s9p1": `9.1. Cancellations prior to approval of the publication by an Administrator: the job price and ${100 - REVISION_PARTE_PCT}% of the commission (with its VAT) are refunded. DOAPP retains the remaining ${REVISION_PARTE_PCT}% of the commission for the review already performed (with the current commission this equals ${REVISION_COMISION_PCT}% of the price, with a minimum of EUR ${REVISION_PISO_EUR}), <b>and the processing cost already charged by the payment gateway is not refunded</b>. The amount is credited as Platform credit.`,
  "s9p2": `9.2. Client cancellations made at least ${P.CANCELACION_CLIENTE_HORAS_ANTES} hours before the start: the job price is returned as credit on the Platform, <b>minus the gateway processing cost</b>, which was already charged and does not return. The publication commission is not refunded once the publication has been approved.`,
  "s9p3": `9.3. <b>Late Client cancellations</b> (less than ${P.CANCELACION_CLIENTE_HORAS_ANTES} hours before the start, or during execution): of the price, less the gateway processing cost, if a Worker had been selected ${PARTE_TRABAJADOR_PCT}% is paid to the Worker for the time they reserved and the rest is returned to the Client as credit; if no Worker had been selected, all of it returns to the Client. In both cases the publication commission is retained. Credit may be used on the Platform at no cost or transferred as per section 7.9.`,
