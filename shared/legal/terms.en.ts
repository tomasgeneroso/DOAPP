...
export const termsEn: Record<string, string> = {
  ...
  "s7p9": "7.9. <b>Credit balance and its transfer.</b> Amounts returned to the Client — from a quote below the published price, a cancellation or a rejected publication — are credited as a balance within the Platform. That balance may be used at no cost on any subsequent publication or hiring; the part of a publication paid with balance does not go through the gateway and carries no processing cost. The Client may also request its transfer to a bank account in their name; <b>the transfer is free of charge</b>, except, where section 9.1 applies, for the share of the commission indicated there, which is disclosed before the request is confirmed. If the Client does not accept that deduction, the balance remains available on the Platform indefinitely.",
  ...
  "s9p1": `9.1. <b>Cancellations with no Worker selected</b> (before the publication is approved, or after approval while no Worker has been selected): the full amount paid, commission included and except for the payment processing cost (section 7.10), is credited as Platform credit, usable at no cost on new publications. If the Client asks to transfer that credit to a bank account, ${REVISION_PARTE_PCT}% of the commission with its VAT is deducted for the review already performed (with the current commission this equals ${REVISION_COMISION_PCT}% of the price, with a minimum of EUR ${REVISION_PISO_EUR}). A publication rejected by an Administrator is settled the same way, and the Administrator must state the reason.`,
