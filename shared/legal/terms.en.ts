...
export const termsEn: Record<string, string> = {
  ...
  "s7p8": "7.8. <b>Minimum contract amount.</b> The minimum amount for a job is ARS $18,000. This minimum is not a fee: it follows from the fact that every contract carries a fixed cost for the Platform — payment processing, custody of funds, identity verification, and dispute handling — that does not decrease when the job amount is smaller. Below that amount, the commission does not cover that cost and the contract cannot be sustained. The minimum is verified when a quote is accepted, which is when a definitive price exists; an \"open to quotes\" publication may be created without a price.",
