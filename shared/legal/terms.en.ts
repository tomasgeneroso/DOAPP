...
export const termsEn: Record<string, string> = {
  ...
  "s7p9": "7.9. <b>Credit balance and its transfer.</b> Amounts returned to the Client — from a quote below the published price, a cancellation or a rejected publication — are credited as a balance within the Platform. That balance may be used at no cost on any subsequent publication or hiring. The Client may also request its transfer to a bank account in their name; in that case <b>the processing cost charged by the payment gateway</b> when that money came in is deducted and, where section 9.1 applies, the share of the commission indicated there. The exact amounts are disclosed before the request is confirmed. If the Client does not accept the deduction, the balance remains available on the Platform indefinitely. The processing cost passed on is the gateway's fee without its VAT; that VAT is borne by the Platform.",
  "s7p10": "7.10. <b>Gateway fees and timing.</b> The processing cost and the time it takes the payment gateway to make funds available depend on the method chosen by the payer (card, account balance, instalments) and on the provider's current conditions, which may change without prior notice to DOAPP. For that reason the exact amount the Worker receives and the estimated transfer date are shown when applying and when the job is confirmed, using the actual data of that payment, and may differ from the indicative values published. DOAPP does not receive any part of that cost: it is passed through as is.",
  ...
  "s9p1": `9.1. <b>Cancellations with no Worker selected</b> (before the publication is approved, or after approval while no Worker has been selected): the full amount paid, commission included, is credited as Platform credit, usable at no cost on new publications. If the Client asks to transfer that credit to a bank account, ${REVISION_PARTE_PCT}% of the commission with its VAT is deducted for the review already performed (with the current commission this equals ${REVISION_COMISION_PCT}% of the price, with a minimum of EUR ${REVISION_PISO_EUR}), together with the payment gateway's processing cost. A publication rejected by an Administrator is settled the same way, and the Administrator must state the reason.`,
  "s9p2": `9.2. <b>Client cancellations with a Worker selected and at least ${P.CANCELACION_CLIENTE_HORAS_ANTES} hours before the start</b>: the job price is credited as Platform credit; the publication commission is not refunded. If the Client transfers that credit to a bank account, the gateway processing cost is deducted as per section 7.9.`,
  "s9p3": `9.3. <b>Late Client cancellations with a Worker selected</b> (less than ${P.CANCELACION_CLIENTE_HORAS_ANTES} hours before the start, or during execution): ${PARTE_TRABAJADOR_PCT}% of the price is paid to the Worker for the time reserved, less the share of the gateway processing cost corresponding to that amount; the rest is credited to the Client as Platform credit, and the corresponding share of the gateway cost is deducted when transferred. The publication commission is retained. If the Worker is the one who cancels — through the function provided in the Platform, before the start or during execution — the Worker receives nothing and is subject to section 9.4, and the Client chooses: keep the publication open with the funds already paid to select another Worker, republish it at a lower price (the difference is credited as Platform credit), or have the full job price credited as Platform credit. In all three cases the publication commission is retained, and the processing cost is deducted when the credit is transferred, as per section 7.9.`,
