...
export const termsEn: Record<string, string> = {
  ...
  "s9p4": `9.4. <b>Cancellation by the Worker of an accepted contract.</b> Cancelling a job already accepted harms the Client and the Platform. Therefore, Worker cancellations within a ${P.CANCELACION_VENTANA_DIAS}-day period carry gradual consequences: the first generates a warning; the second makes visible on the Worker's profile, for ${P.CANCELACION_MARCA_VISIBLE_DIAS} days, a mark indicating they cancelled accepted jobs; the third or subsequent ones suspend the ability to apply for new jobs for ${P.CANCELACION_SUSPENSION_DIAS} days. Ongoing contracts are not affected by the suspension. Giving timely notice that the job cannot be performed, through the feature provided on the Platform, counts as a cancellation for these purposes, but allows the Client to decide how to dispose of their money without delay.`,
